function varargout = Guide_results_valid(varargin)
% GUIDE_RESULTS_VALID MATLAB code for Guide_results_valid.fig
%      GUIDE_RESULTS_VALID, by itself, creates a new GUIDE_RESULTS_VALID or raises the existing
%      singleton*.
%
%      H = GUIDE_RESULTS_VALID returns the handle to a new GUIDE_RESULTS_VALID or the handle to
%      the existing singleton*.
%
%      GUIDE_RESULTS_VALID('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIDE_RESULTS_VALID.M with the given input arguments.
%
%      GUIDE_RESULTS_VALID('Property','Value',...) creates a new GUIDE_RESULTS_VALID or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Guide_results_valid_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Guide_results_valid_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Guide_results_valid

% Last Modified by GUIDE v2.5 12-Jul-2020 11:28:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Guide_results_valid_OpeningFcn, ...
                   'gui_OutputFcn',  @Guide_results_valid_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Guide_results_valid is made visible.
function Guide_results_valid_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Guide_results_valid (see VARARGIN)
...
% Choose default command line output for Guide_results_valid
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Guide_results_valid wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Guide_results_valid_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global SUH_method BF_flag x_par_valid dt calib_flag valid_flag ungauged_flag;
calib_flag =0;
valid_flag =1;
ungauged_flag =0;
...
if val_res > 0
    if handles.check_unit_hidro.Value == 1
        if handles.radio_fig.Value ==1
            make_figure('Validation','Time(h)','Runoff (m^3/s)',BF_flag)
        else
            % Load data
    ...
    if SUH_method == 10
        if handles.check_hill_rivers.Value == 1
            ...
            figure
            subplot(1,2,1)
            hill_n = hill./x_par_valid(2)*dt/(3600);  %[hours]
            hill_n(hill_n==0)=nan;
            imh = image(hill_n);
            axis 'equal'
            ...
            river_n = (river./x_par_valid(1))*dt/(3600);  %[hours]
...
global dt CN_valid Ia_valid SUH_method SUH_string BF_flag x_par_valid NSE_valid P1_text P2_text;
...
if handles.check_unit_hidro.Value == 1
    ...
    rg_valid_data = questdlg(['Would you like to save the validation results?'], ...
        'Save file', ...
        'Save file','Exit','Exit');
...
if handles.check_hill_rivers.Value == 1
     ...
     switch rg2
         case 'Save file'
             ...
             if path2 ~= 0
                 load('data\data_base\Validation\temp\hill','hill')
                 travel_time_h = hill./x_par_valid(2)*dt/(3600);  %[hours]
...
rg_valid = questdlg(['Would you like to save parameters and statistics?'], ...
    'Save file', ...
    'Save file','Exit','Exit');
switch rg_valid
    case 'Save file'      
        ...
        if SUH_method == 10 || SUH_method == 7 || SUH_method == 6 || SUH_method == 5 || SUH_method == 3 || SUH_method == 2
            ...
            if path ~= 0
                ...
                fprintf(path_file,'***************************************************************\r\n');
                fprintf(path_file,['                      ',char(cell(SUH_string)),'(Validation)\r\n']);
                fprintf(path_file,'***************************************************************\r\n');
                fprintf(path_file,'%6s %32s\r\n','Parameters','Values');
                fprintf(path_file,'%5s %35s\r\n',P1_text,num2str(x_par_valid(1)));
                fprintf(path_file,'%5s %35s\r\n',P2_text,num2str(x_par_valid(2)));
                fprintf(path_file,'%5s %21s\r\n','Curve Number (CN): ',num2str(CN_valid));
                fprintf(path_file,'%5s %11s\r\n','Initial abstraction (Ia): ',num2str(Ia_valid));
                fprintf(path_file,'--> Validation statistics\r\n');
                fprintf(path_file,'%5s %10s\r\n','Nash–Sutcliffe model efficiency coefficient (NSE):',num2str(NSE_valid));
                fprintf(path_file,'***************************************************************\r\n');
                fclose(path_file);
            end
        else
